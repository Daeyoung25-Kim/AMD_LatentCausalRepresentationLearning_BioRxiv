...
ranksums(zz[dn_tr==0,0],zz[dn_tr==1,0],alternative="greater")
#brunnermunzel(zz[dn_tr==0,0],zz[dn_tr==1,0],alternative="greater")

from sklearn.feature_selection import mutual_info_classif, mutual_info_regression
mutual_info_classif(zz[:,0].reshape(-1,1),dn_tr,n_neighbors=5)
...
#Visualizing Difference in Intensity (Example: Z4 and Hemorrhage correspondence)
set1 = [-11,-10,-9,-8,-7] #For ZO
set2 = [0,1,2,3,4] #For Z4
set3 = [2,3,4,5,6] #For Z5

distangle_1 = np.array(z11[tg[52],:]) #19,35
distangle_1[4] = set2[0]
distg1 = tf.reshape(tf.convert_to_tensor(distangle_1, dtype=tf.float32),[1,8])
distg1 = CVAE.dec(distg1, sigmoid=True)


distangle_2 = np.array(z11[tg[52],:])
distangle_2[4] = set2[4]
distg2 = tf.reshape(tf.convert_to_tensor(distangle_2, dtype=tf.float32),[1,8])
distg2 = CVAE.dec(distg2, sigmoid=True)

diff = np.abs(distg1-distg2)
diff = (diff-np.min(diff))/(np.max(diff)-np.min(diff))

diff = cv2.cvtColor(np.abs(np.array(diff))[0,:,:,:], cv2.COLOR_BGR2GRAY)
plt.imshow(diff,cmap="gray") #Bright part denotes high difference
plt.grid()

plt.imshow(X_nw2[tg[52],:,:,:])#Original image. check location of hemorrhage
plt.grid()
plt.show()
